...
Deferred.prototype.exec = function(cb) {
  ...
  if(!cb) {
    cb = function(){}; // noop callback if no callback was given
  }
  ...
  var promisecb = function(err, results){
    if (err)
      deferred.reject(err);
    else
  ...
  // Normalize callback/switchback
  promisecb = normalize.callback(promisecb);

  // Set up arguments + callback
  var args = [this._criteria, promisecb];
...
// preferably this function will be deprecated
